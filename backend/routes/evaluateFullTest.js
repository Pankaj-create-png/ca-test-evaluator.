...
router.post('/evaluate-full-test', async (req, res) => {
  try {
    ...
    // Call Gemini API for full test paper with request queue, retries & backoff
    ...
    console.error(`[Evaluate Full Test API Error] Request failed. Status: ${status || 'N/A'}, Message: "${errorMsg}"`, error);
    ...
    return res.status(500).json({
      error: errorMsg || 'An error occurred while evaluating the full test paper.',
